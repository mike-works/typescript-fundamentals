...
function wrappedValue<T>(x: T): { value: T } {
  return {
    value: x
  };
}
wrappedValue(5); // { value:  number }
wrappedValue(["foo"]); // { value: string }

// type param determines the type of x
interface WrappedValue<T> {
  value: T;
}
const wrappedStr: WrappedValue<string> = { value: "abc" };
...
function arrayToDict<T extends WithId = WithIdPlus>(array: T[]): Dict<T> {
...
const myDict = arrayToDict(items)["a"].id;
/**
 *    a: { id: "a", value: "first", lisa: "Huang" },
 *    b: { id: "b", value: "second" }
 *
 */
